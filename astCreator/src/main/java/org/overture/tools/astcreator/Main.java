...
public class Main
{
		...
		try
		{
			...
			switch (run)
			{
				...
				case Nested:
				{
					...
					Environment env1 = create(null, new FileInputStream(input1), generated, true, GENERATE_VDM);
	...
	/**
	 * Given an Abstract Syntax Tree source (ast1) defining a base tree and an Abstract Syntax Tree (ast2) defining a
	 * proper extension of that base tree this method crates the source for both trees.
	 * 
	 * @param ast1
	 *            - the base tree
	 * @param ast2
			...
			boolean extOnly,boolean write) throws Exception
	{
		...
		Environment envExtOnly = generator.generate(ast2ToString, ast2, extendName, false, false);
