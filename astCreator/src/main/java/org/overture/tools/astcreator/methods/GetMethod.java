...
public class GetMethod extends Method
{
	...
	protected void prepare(Environment env)
	{
		...
		javaDoc = "\t/**\n";
		// javaDoc +=
		// "\t * Returns the {@link "+f.getType()+"} node which is the {@code "+f.getName()+"} child of this {@link "+classDefinition.getName().getName()+"} node.\n";
		javaDoc += "\t * @return the {@link "
				+ NameUtil.stripGenerics(f.getType(true, env))
				+ "} node which is the {@code " + f.getName(env)
				+ "} child of this {@link "
				+ classDefinition.getName().getName() + "} node\n";
		javaDoc += "\t*/";

		this.name = "get" + NameUtil.javaClassName(f.getName(env));
